#Cuestion 1 Hito de programacion FRANK_ESPIN
...
opcion = input("Ingrese su opcion (cuadrado o rectangulo): ") #Presentamos las opciones cuadrado rectangulo
...
#si pone un numero no valido imprimira "Ingresa opcion valida"
...
def jugar():
    ...
    while True:
        try:
            opcion_usuario = int(input("Introduce un numero (1: piedra, 2: papel, 3: tijera): "))#Pedimos una opcion al usuario
            if opcion_usuario in opciones: 
                break
            else:
                print("Opcion no valida. Ingresa una opcion correcta.")
        except ValueError:
            print("Ingresa una opcion valida.")# Imprimira esto si el usuario a ingresado un numero que no es (1,2 o 3)

    opcion_maquina = random.randint(1, 3) #Pedimos una opcion a la maquina
    print(f"\nTu opcion: {opciones[opcion_usuario]}")
    print(f"Opcion maquina: {opciones[opcion_maquina]}")

    if opcion_usuario == opcion_maquina: 
        print("Empate")#Si las 2 opciones son  iguales sera empate
...
def main():
    ...
    while True:                  #CREAMOS EL MENU PARA EL USUSARIO
        ...
        opcion = input( "Elige una opcion (1-4): ")        #EL USUSARIO PUEDE ESCOGER ENTRE LOS NUMEROS DEL MENU
        if opcion == '1':
            cantidad = float(input("Ingresa la cantidad de dinero que quiere ingresar: "))
            if cantidad < 0:
                print("No se pueden ingresar cantidades negativas.")  #SE PRODUCE DEBIDO A QUE A INGRESADO UNA CANTIDAD <0 Y SALTARA EL MENSAJE CORRESPONDIENTE 
            else:
                saldo += cantidad
            ...
            if cantidad < 0:
                print("No se pueden retirar cantidades negativas.") #SE PRODUCE DEBIDO A QUE EL USUSARIO QUIERE RETIRAR CANTIDADES NEGATIVAS Y SALTARA EL MENSAJE CORRESPONDIENTE 
            elif saldo - cantidad < 0:
                print("No puedes retirar mas cantidad de la disponible .") #SE PRODUCE DEBIDO A QUE EL USUSARIO QUIERE MAS CANTIDAD DE LAS DISPONIBLES Y SALTARA EL MENSAJE CORRESPONDIENTE
            else:
                saldo -= cantidad
            ...
            print("Usted ha escogido la opcion salir (SALIENDO...)")  #HA ELEGIDO LA OPCION SAIR
            break
